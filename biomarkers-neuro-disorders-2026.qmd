...
`PACKAGE::OBJECT` grabs the `OBJECT` from the `PACKAGE`. So `SUBJINFO` will now be different than `A4LEARN::SUBJINFO`

:::

## Build the longitudinal PACC analysis set

```{r organize-a4learn-long-data}
#| echo: true
...
The pipe operator (`|>` or `%>%`) send the results to the next function. The RStudio shortcut for it is:
...
```{r sim-batch-effect, results = 'hide', fig.show='hide'}
#| echo: true
set.seed(20200225)
...
Both mean (or "location") and variance (or "scale", "spread") vary by batch.

:::

## Testing for batch effects on the mean

```{r anova-sim-batch-effect}
#| echo: true
...
Adjust for batch effects using the ComBat empirical Bayes framework [@johnson2007adjusting; @sva], developed for genomics data. This method estimates batch-specific location and scale parameters (as in `gls` fit above) and adjusts the data accordingly.
...
* `sva::ComBat` is designed for high throughput data and expects more than 1 marker. Here we create a pseudo marker by duplicating the biomarker with slight "`jitter`".
...
::: incremental
-   randomize samples to plates when possible to avoid confounding batch effects with effects of interest (ComBat cannot fix this)
-   visualize by plate or batch before modeling
-   use techniques like ComBat to adjust for known technical issues
...
::: callout-note
* "beeswarm" helps show raw data and reduce overplotting
* "violin plot" shows shape of distribution ("density plot")
* "box and whiskers plot" shows quartiles, indicates outliers
:::
...
* trend lines by group are from "ordinary least squares" linear model
* distinct linear trends in LEARN vs A4
* no hard CL threshold for A4 inclusion due to visual reads
...
Effect of APOE $\epsilon4$ not significant after amyloid PET and p-tau217 adjustment
...
## Distribution of baseline amyloid PET (SUVR)
...
-   last observation carried forward (LOCF) is biased
...
Variance of subject-specific random intercepts and slopes; and variance of the residuals
...
-   the `ns(ADURW, df = 2) × Group` interaction estimates differential longitudinal change by group
:::
...
-   smaller AIC/BIC indicates more parsimonious fit
:::
...
-   **Technical artifact problem**\
    Visualize first, address nuisance (ComBat) or adjust in the model

-   **Baseline biomarker and cognition question**\
    Use regression, preserve continuous scales (do not dichotomize if possible)
...
In the event of a successful treatment effect, multiple imputation of missing values can be used to "stress test" the result and ask:

> How wrong do the values imputed for treated individuals, under the assumption of missing at random, have to be before our conclusion is reversed?

This "delta method" assumes $\delta$% more pessimistic imputed values and reports the smallest $\delta$ such that the p-value is no longer significant.
...
Multiple Imputation by Chained Equations (MICE) [@buuren2010mice; @vanbuuren2018flexible]

# 2. Analyze each of 5 complete datasets
...
Imputed PACC_ch_066 values are only changed for those in active group
